...
export class CoordinatorService implements Coordinator {
  ...
  async proxy(request: Empty): Promise<ProxyResponse> {
    console.log("proxy");
    const ctx = restate.useContext(this);
    const receiverClient = new ReceiverClientImpl(ctx);

    let uuid = await ctx.sideEffect(async () => uuidv4());

    let pong = await receiverClient.ping({key: uuid});
  ...
  async complex(request: ComplexRequest): Promise<ComplexResponse> {
    ...
    await ctx.sleep(request.sleepDuration?.millis!);

    let key = await ctx.sideEffect(async () => uuidv4());
    ...
    // getValue.
    await ctx.inBackground(() => receiverClient.setValue({key, value: request.requestValue}));
    let response = await receiverClient.getValue({key});
