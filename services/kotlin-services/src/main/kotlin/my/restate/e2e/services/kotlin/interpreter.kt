...
suspend fun <T> checkAwaitable(actual: Awaitable<T>, expected: T, cmdIndex: Int, command: Command) {
  val result = actual.await()
  if (result != expected) {
    throw TerminalException(
        "Awaited promise mismatch. got '$result' expected '$expected'; command at index $cmdIndex was $command")
  }
}

suspend fun <T> checkAwaitableFails(actual: Awaitable<T>, cmdIndex: Int, command: Command) {
  try {
    actual.await()
  ...
  throw TerminalException(
      "Awaited promise mismatch. should fail but instead got ${actual.await()}; command at index $cmdIndex was $command")
}
...
class ObjectInterpreterImpl(private val layer: Int) : ObjectInterpreter {
  ...
  fun interpreterId(ctx: SharedObjectContext): InterpreterId {
