...
public class ServiceRunner {
    ...
    private ServiceRunner(List<? extends BindableService> bindableServices, int port) {
        this.serverServiceDefinitions = bindableServices.stream().map(RestateGrpcInterceptors::from).collect(Collectors.toList());
        this.port = port;
    }
    ...
    public static ServiceRunner create(BindableService... bindableServices) {
        return createWithPort(Optional.ofNullable(System.getenv("PORT"))
                .map(Integer::parseInt).orElse(8080), bindableServices);
    }
